import { useContext, useEffect, useState } from "react";
import { Link, NavLink } from "react-router-dom";
import { Tooltip } from "react-tooltip";
import { AuthContext } from "../AuthProvider/AuthProvider";
import { toast } from "react-toastify";
import axios from "axios";


const Navbar = () => {
    const { user, logOut } = useContext(AuthContext);
  const navLinks = (
    <>
      <li>
        <NavLink to="/">Home</NavLink>
      </li>
      
      {user ? (
        <>
          <li>
            <NavLink to="/add-book">Add Book</NavLink>
          </li>
          <li>
            <NavLink to="/all-books">All Books</NavLink>
          </li>
          <li>
            <NavLink to="/borrowed-books">Borrowed Books</NavLink>
          </li>
        </>
      ) : (
        " "
      )}
    </>
  );

    const handleLogOut = () => {
       logOut()
         .then(() => {
           toast.success("Sign Out Successful");
          axios("https://dream-library-server.vercel.app/logout", {withCredentials: true})
          .then(res=> {
            console.log(res)
          })
         })
         .catch((error) => {
           console.log(error.message);
           toast.error("Something went wrong");
         });
    }

  const [theme, setTheme] = useState(
    localStorage.getItem("theme") ? localStorage.getItem("theme") : "light"
  );

  useEffect(() => {
    localStorage.setItem("theme", theme);
    const localTheme = localStorage.getItem("theme");
    document.querySelector("html").setAttribute("data-theme", localTheme);
  }, [theme]);

  const handleToggle = (e) => {
    if (e.target.checked) {
      setTheme("dark");
    } else {
      setTheme("light");
    }
  };

  return (
    <div className="navbar py-5 bg-violet-700 text-white z-20">
      <div className="navbar-start">
        <div className="dropdown z-20">
          <div tabIndex={0} role="button" className="btn btn-ghost lg:hidden">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-5 w-5"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth="2"
                d="M4 6h16M4 12h8m-8 6h16"
              />
            </svg>
          </div>
          <ul
            tabIndex={0}
            className="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-violet-700 rounded-box underline w-52"
          >
            {navLinks}
          </ul>
        </div>
        <img
          src="https://i.ibb.co/4Z6hbhW/public-library.png"
          alt=""
          className="w-[30px] md:w-[40px]"
        />
        <Link to="/" className=" ml-3 text-base md:text-2xl font-bold">
          Book Nest
        </Link>
      </div>
      <div className="navbar-center hidden lg:flex">
        <ul className="menu menu-horizontal underline px-1">{navLinks}</ul>
      </div>
      <div className="navbar-end  md:gap-3">
        {user ? (
          <div className=" flex gap-2 justify-center items-center">
            <p className="text-sm  md:text-xl font-bold">{user?.displayName}</p>
            <Tooltip id="my-tooltip" />
            <div
              tabIndex={0}
              role="button"
              data-tooltip-id="my-tooltip"
              // data-tooltip-content={user?.displayName}
              data-tooltip-place=""
              className="tooltip avatar z-40"
            >
              <div className="w-8 md:w-10 rounded-full">
                <img
                  className="w-full h-full"
                  alt="Tailwind CSS Navbar component"
                  src={user?.photoURL}
                />
              </div>
            </div>

            {/* theme */}
            <div>
              <label className="swap swap-rotate">
                {/* this hidden checkbox controls the state */}
                <input type="checkbox" onChange={handleToggle} />

                {/* sun icon */}
                <svg
                  className="swap-on fill-current md:w-10 w-8 h-8 md:h-10"
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                >
                  <path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" />
                </svg>

                {/* moon icon */}
                <svg
                  className="swap-off fill-current w-8 md: 10 h-8 md:h-10"
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                >
                  <path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" />
                </svg>
              </label>
            </div>
            <button
              onClick={handleLogOut}
              className="font-bold rounded-md text-sm md:text-xl bg-violet-500 md:py-3 p-1 md:px-5 text-white"
            >
              LogOut
            </button>
          </div>
        ) : (
          <div className="flex gap-2">
            <Link
              to="/login"
              className="font-bold rounded-md bg-violet-500 md:py-3 md:px-5 p-2 text-white"
            >
              Login
            </Link>
          </div>
        )}
      </div>
    </div>
  );
};

export default Navbar;
